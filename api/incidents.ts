...
import { db, type Incident } from '../lib/db/index.js';

export default async function incidentsHandler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method === 'GET') {
    const incidents = Array.from(db.incidents.values()).sort(
